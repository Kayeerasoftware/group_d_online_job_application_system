...
<div class="bg-gradient-to-br from-gray-50 to-gray-100 p-3 md:p-6 min-h-screen">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 md:gap-3 mb-4 md:mb-6">
        ...
        <div class="bg-gradient-to-r from-teal-50 to-teal-100 rounded-lg shadow-md p-2 md:p-3 hover:shadow-lg hover:scale-105 transition-all duration-300 border border-teal-200">
            <div class="flex items-center gap-2 md:gap-3">
                ...
                <div class="flex-1">
                    <p class="text-xs md:text-sm text-teal-600 font-bold leading-tight">Messages</p>
                    <h3 class="text-xl md:text-3xl font-bold text-gray-900 leading-tight">{{ $messageNotifications ?? 0 }}</h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-4 mb-6 flex gap-2 overflow-x-auto">
        <a href="{{ route('employer.notifications') }}" class="px-4 py-2 {{ !request('type') ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }} rounded-lg font-semibold text-sm whitespace-nowrap transition">
            All
        </a>
        <a href="{{ route('employer.notifications', ['type' => 'application']) }}" class="px-4 py-2 {{ request('type') === 'application' ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }} rounded-lg font-semibold text-sm whitespace-nowrap transition">
            <span class="{{ request('type') === 'application' ? 'text-white' : 'text-blue-600' }} font-bold">Applications</span>
        </a>
        <a href="{{ route('employer.notifications', ['type' => 'interview']) }}" class="px-4 py-2 {{ request('type') === 'interview' ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }} rounded-lg font-semibold text-sm whitespace-nowrap transition">
            <span class="{{ request('type') === 'interview' ? 'text-white' : 'text-purple-600' }} font-bold">Interviews</span>
        </a>
        <a href="{{ route('employer.notifications', ['type' => 'message']) }}" class="px-4 py-2 {{ request('type') === 'message' ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }} rounded-lg font-semibold text-sm whitespace-nowrap transition">
            <span class="{{ request('type') === 'message' ? 'text-white' : 'text-teal-600' }} font-bold">Messages</span>
        </a>
        <a href="{{ route('employer.notifications', ['type' => 'system']) }}" class="px-4 py-2 {{ request('type') === 'system' ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }} rounded-lg font-semibold text-sm whitespace-nowrap transition">
                        ...
                        <div class="flex-shrink-0">
                            @php
                                ...
                                $iconClass = $notificationType === 'application' ? 'bg-blue-100' : 
                                            ...
                                            ($notificationType === 'message' ? 'bg-teal-100' : 'bg-gray-100'));
                                $iconColor = $notificationType === 'application' ? 'text-blue-600' : 
                                            ($notificationType === 'interview' ? 'text-purple-600' : 
                                            ($notificationType === 'message' ? 'text-teal-600' : 'text-gray-600'));
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center justify-between flex-wrap gap-2">
                                ...
                                <div class="flex gap-2">
                                    @if(!$notification->read_at)
                                        <form action="{{ route('employer.notifications.read', $notification) }}" method="POST" class="inline">
                                            @csrf
                                    ...
                                    @endif
                                    <form action="{{ route('employer.notifications.destroy', $notification) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?');">
                                        @csrf
                                        @method('DELETE')
