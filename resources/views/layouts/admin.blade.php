...
<head>
    ...
    <title>@yield('title', 'Admin Dashboard') - Job Application System</title>
...
<body class="bg-gray-50" style="overflow-y: auto; overflow-x: hidden; scrollbar-width: none;" x-data="{ 
    ...
    adminProfile: {
        ...
        name: {{ Js::from($currentUser->name ?? 'Administrator') }},
        role: 'Administrator',
        email: {{ Js::from($currentUser->email ?? 'admin@example.com') }},
        phone: {{ Js::from($currentUser->phone ?? '+256 700 000 000') }},
        initials: {{ Js::from($initials) }}
    },
