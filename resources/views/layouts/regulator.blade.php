...
<head>
    ...
    <title>@yield('title', 'Regulator Dashboard') - Job Application System</title>
...
<body class="bg-gray-50" style="overflow-y: auto; overflow-x: hidden; scrollbar-width: none;" x-data="{ 
    ...
    regulatorProfile: {
        ...
        name: {{ Js::from($currentUser->name ?? 'Regulator') }},
        role: 'Regulator',
        email: {{ Js::from($currentUser->email ?? 'regulator@example.com') }},
        phone: {{ Js::from($currentUser->phone ?? '+256 700 000 000') }}
    },
    profilePicture: {{ Js::from($currentUser->profile_picture_url ?? null) }}
}">
    @include('partials.regulator-topnav')
    @include('partials.regulator-sidenav')
