<nav class="topnav no-print fixed top-0 left-0 right-0 h-12 bg-gradient-to-r from-violet-200 via-blue-200 to-cyan-200 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 backdrop-blur-xl border-b border-violet-300 dark:border-gray-600 z-50 shadow-2xl ring-2 ring-gradient-to-r ring-from-violet-400 ring-to-cyan-400 ring-opacity-80 shadow-violet-500/30 dark:shadow-gray-900/30 transition-colors duration-300">
    <div class="nav-container flex items-center justify-between h-full px-6 max-w-full mx-auto">
        <div class="nav-left flex items-center gap-0">
            ...
            <div class="flex items-center px-3 py-1.5 bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg border border-blue-200 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:border-blue-300" @click="showLogoModal = !showLogoModal" :class="showLogoModal ? 'ring-2 ring-blue-400' : ''">
                <h2 class="m-0 bg-gradient-to-br from-blue-600 to-purple-600 bg-clip-text text-transparent text-xs font-bold hidden md:block cursor-pointer">ONLINE JOB APPLICATION SYSTEM</h2>
            </div>
        </div>

        <div class="nav-center flex-1 flex justify-center">
            <div class="flex items-center gap-2 font-bold text-sm tracking-tight px-6 py-1 rounded-xl border transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md cursor-pointer hidden lg:flex" style="background: linear-gradient(135deg, #dbeafe, #bfdbfe); border-color: #3b82f6;" @click="showSeekerModal = !showSeekerModal" :class="showSeekerModal ? 'ring-2 ring-blue-400' : ''">
                <div class="bg-white/20 p-0.5 rounded-lg backdrop-blur-sm">
                    <i class="fas fa-user-tie text-sm drop-shadow-md" style="color: #2563eb;"></i>
                </div>
                <span class="text-sm font-bold whitespace-nowrap" style="background: linear-gradient(135deg, #2563eb, #3b82f6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">Job Seeker Dashboard</span>
            </div>
        </div>
        ...
        <div class="nav-right flex items-center gap-2">
            ...
            <a href="{{ route('seeker.notifications') }}" class="nav-btn relative flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 border border-blue-200 px-3 py-1.5 rounded-xl cursor-pointer transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:border-blue-300" title="Notifications">
            ...
            <button @click="showProfileDropdown = !showProfileDropdown" class="flex items-center gap-2 px-3 py-1.5 rounded-xl cursor-pointer transition-all duration-300 bg-gradient-to-br from-blue-50 to-purple-50 border border-blue-200 hover:-translate-y-0.5 hover:shadow-md hover:border-blue-300">
                <span class="bg-gradient-to-br from-blue-600 to-purple-600 bg-clip-text text-transparent text-xs font-bold tracking-tight hidden lg:inline" x-text="'Welcome, ' + seekerProfile.name">Welcome, Job Seeker</span>
...
<div x-show="showProfileDropdown" @click.away="showProfileDropdown = false" x-transition class="fixed top-14 right-6 w-44 bg-white rounded-xl shadow-2xl z-40 border border-blue-100 overflow-hidden" style="display: none;">
    ...
    <div class="pt-6 pb-3 px-3 text-center bg-gradient-to-br from-purple-500 to-pink-600 relative">
        ...
        <div class="relative">
            ...
            <p class="text-sm font-bold text-white truncate" x-text="seekerProfile.name">Job Seeker</p>
            <p class="text-xs text-purple-100">Job Seeker</p>
        </div>
    </div>
    
    <div class="py-1.5">
        <a href="{{ route('seeker.profile') }}" @click="showProfileDropdown = false" class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-purple-50 transition group">
        ...
        <a href="{{ route('seeker.settings') }}" @click="showProfileDropdown = false" class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-pink-50 transition group">
        ...
        <div class="flex items-center justify-between px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
            ...
            <button @click="darkMode = !darkMode" class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors" :class="darkMode ? 'bg-purple-600' : 'bg-gray-300'">
...
<div x-show="showLogoModal" x-transition @click.away="showLogoModal = false" x-cloak class="fixed top-12 left-0 right-0 bg-white border-b border-gray-200 shadow-xl z-[100] p-6">
    <div class="max-w-md mx-auto text-center">
        ...
        <h2 class="text-lg font-bold bg-gradient-to-br from-blue-600 to-purple-600 bg-clip-text text-transparent mb-2">JOB APPLICATION SYSTEM</h2>
...
<div x-show="showSeekerModal" x-transition @click.away="showSeekerModal = false" x-cloak class="fixed top-12 left-0 right-0 bg-white border-b border-gray-200 shadow-xl z-[100] p-6">
    <div class="max-w-md mx-auto text-center">
        ...
        <h2 class="text-lg font-bold bg-gradient-to-br from-blue-600 to-indigo-600 bg-clip-text text-transparent mb-2">Job Seeker Dashboard</h2>
        <p class="text-sm text-gray-600 mb-1">Manage your applications and profile</p>
        <p class="text-xs text-gray-400">Track your job search progress</p>
    </div>
</div>
