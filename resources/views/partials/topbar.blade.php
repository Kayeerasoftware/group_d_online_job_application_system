...
<header class="app-chrome sticky top-0 z-30 border-b border-white/10 bg-[#1a1714] backdrop-blur-xl {{ safe_auth_check() ? 'md:pl-72 xl:pl-80' : '' }}">
    <div class="flex w-full items-center gap-4 px-4 py-4 md:px-6 lg:px-8">
        @if (safe_auth_check())
            <button
                type="button"
        ...
        <a href="{{ safe_auth_check() ? route('dashboard') : route('home') }}" class="flex min-w-0 items-center gap-3">
            @if (safe_auth_check())
                ...
                <span class="hidden min-w-0 flex-col sm:flex">
                    <span class="truncate text-sm font-semibold text-white">JobLink Uganda</span>
                    <span class="truncate text-xs text-white/55">Hiring workspace</span>
                </span>
            @else
                <img
                    ...
                    loading="eager"
                    decoding="async"
                    class="topbar-brand__logo topbar-brand__logo--home"
                >
                @if (request()->routeIs('home'))
                    <span class="hidden min-w-0 flex-col sm:flex">
                        <span class="truncate text-sm font-semibold text-white">JobLink Uganda</span>
                        <span class="truncate text-xs text-white/55">Find work faster</span>
                    </span>
                @endif
            @endif
        </a>

        @if (request()->routeIs('home') && ! safe_auth_check())
            <nav class="hidden items-center gap-1 lg:flex">
                <a href="{{ route('jobs.index') }}" class="topbar-home-link">Browse Jobs</a>
                <a href="{{ route('register') }}" class="topbar-home-link">Create Account</a>
                <a href="{{ route('login') }}" class="topbar-home-link">Sign In</a>
            </nav>
        @else
        ...
        <div class="ml-auto flex items-center gap-3">
            ...
            @else
                @if($user?->isSeeker())
                    <a href="{{ route('seeker.saved-jobs.index') }}" class="hidden items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white/85 transition hover:bg-white/10 lg:inline-flex">
                        <svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
