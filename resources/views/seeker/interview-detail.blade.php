...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-3 md:p-6">
    <!-- Back Button -->
    <div class="mb-4">
        <a href="{{ route('seeker.interviews') }}" class="inline-flex items-center gap-2 text-teal-600 hover:text-teal-700 font-semibold">
            <i class="fas fa-arrow-left"></i>Back to Interviews
        </a>
    </div>

    <!-- Header -->
    <div class="bg-white rounded-xl shadow-lg p-6 mb-6 border border-teal-100">
        <div class="flex items-start justify-between gap-4">
            <div class="flex-1">
                ...
                <p class="text-lg text-gray-600">{{ $application->job->company_name ?? 'Company' }}</p>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                    ...
                    <div>
                        ...
                        <p class="text-lg font-semibold">
                            ...
                            <span class="text-teal-600">Upcoming</span>
                            @else
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 mb-6 border border-gray-100">
                <h2 class="text-xl font-bold text-gray-900 mb-4 flex items-center">
                    <i class="fas fa-info-circle text-teal-600 mr-2"></i>Interview Details
                </h2>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
                <h2 class="text-xl font-bold text-gray-900 mb-4 flex items-center">
                    <i class="fas fa-comments text-teal-600 mr-2"></i>Communication
                </h2>
                ...
                <div class="bg-gray-50 rounded-lg p-4 mb-4 h-96 overflow-y-auto space-y-4" id="messagesContainer">
                    ...
                    <div class="flex {{ $comm->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-xs {{ $comm->sender_id === auth()->id() ? 'bg-teal-100 text-teal-900' : 'bg-gray-200 text-gray-900' }} rounded-lg p-3">
                            ...
                            @elseif($comm->message_type === 'reschedule_request')
                            <div class="text-sm">
                                <p class="font-semibold mb-1">Reschedule Request</p>
                                <p>{{ $comm->message }}</p>
                                <p class="mt-1 text-xs">Proposed: {{ \Carbon\Carbon::parse($comm->metadata['proposed_date'])->format('M d, Y h:i A') }}</p>
                            </div>
                            @elseif($comm->message_type === 'feedback')
                            <div class="text-sm">
                                <p class="font-semibold mb-1">Interview Feedback</p>
                                <p>{{ $comm->message }}</p>
                                <p class="mt-1 text-xs">Outcome: <span class="font-semibold">{{ ucfirst($comm->metadata['outcome']) }}</span></p>
                            </div>
                ...
                <!-- Message Input -->
                <form method="POST" action="{{ route('seeker.interviews.message', $application->id) }}" class="space-y-3">
                    @csrf
                    <textarea name="message" placeholder="Type your message..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500" rows="3" required></textarea>
                    <div class="flex gap-2">
                        <button type="submit" class="flex-1 px-4 py-2 bg-gradient-to-r from-teal-600 to-cyan-600 text-white rounded-lg hover:shadow-lg transition-all duration-300 font-semibold">
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
                <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center">
                    <i class="fas fa-building text-teal-600 mr-2"></i>Employer
                </h3>
                <div class="space-y-3">
                    <div>
                        <p class="text-sm text-gray-600">Company</p>
                        <p class="font-semibold text-gray-900">{{ $employer->name }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Email</p>
                        <p class="font-semibold text-gray-900">{{ $employer->email }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
                ...
                <div class="space-y-2">
                    ...
                    <button onclick="openRescheduleModal()" class="w-full px-4 py-2 bg-white text-amber-600 border border-amber-200 rounded-lg hover:bg-amber-50 transition-all duration-300 font-semibold text-sm">
                        <i class="fas fa-calendar-times mr-2"></i>Request Reschedule
                    </button>
                    @endif

                    @if($application->interview_link)
                    <a href="{{ $application->interview_link }}" target="_blank" class="block w-full px-4 py-2 bg-gradient-to-r from-teal-600 to-cyan-600 text-white rounded-lg hover:shadow-lg transition-all duration-300 font-semibold text-sm text-center">
                        <i class="fas fa-video mr-2"></i>Join Interview
                    </a>
                    @endif

                    @if($application->scheduled_date <= now() && !$application->feedback)
                    <button onclick="openFeedbackModal()" class="w-full px-4 py-2 bg-white text-emerald-600 border border-emerald-200 rounded-lg hover:bg-emerald-50 transition-all duration-300 font-semibold text-sm">
                        <i class="fas fa-star mr-2"></i>View Feedback
                    </button>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex gap-3">
                        <div class="w-2 h-2 bg-teal-600 rounded-full mt-1.5 flex-shrink-0"></div>
                        <div>
...
<div id="rescheduleModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
        <h2 class="text-2xl font-bold text-gray-900 mb-4">Request Reschedule</h2>
        <form id="rescheduleForm" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Proposed Date & Time</label>
                <input type="datetime-local" name="proposed_date" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Reason</label>
                <textarea name="reason" placeholder="Why do you need to reschedule?" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500" rows="3"></textarea>
            </div>
            <div class="flex gap-2">
